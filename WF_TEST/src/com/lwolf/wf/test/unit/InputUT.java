...
public class InputUT {
	...
	private final ArgumentCaptor<InputEntity> inputEntityArg = ArgumentCaptor.forClass(InputEntity.class);
	...
	public void shouldCreateInputs() throws Exception {
		...
		process.name = "shouldCreateInputs";
		
		// when
		processService.add(process);
		
		// then
		verify(inputDaoMock, atLeastOnce()).create(inputEntityArg.capture());
		assertEquals("InputDao.create called with improper processId value", new Integer(100), inputEntityArg.getValue().processId);
	}
